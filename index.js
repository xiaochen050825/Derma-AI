...
const geminiKeys = [
    "AIzaSyCMUa7uMiJBS7OXLc7CYpmvrLIpXnXGjMg",
    "AIzaSyAWdoyFKneKlkhTxrWe4bONVWXSphAPgMo",
    "AIzaSyCW-5T83wbGk7k-rFwoDh5Ut1R1A_Iq6gw",
    "AIzaSyAmsqBYedElCoblmbIipmI_zm_kkw55F8U",
    "AIzaSyB1FDOlsDCMnxdp60p-hWzHSbrhfuEG9Zk",
    "AIzaSyBnHDrG2PHsBsBcZTFaUfjNbiVK-GVY2wo"
];
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        const systemInstruction = `
            You are DermaAI Copilot, a professional dermatology screening assistant focusing specifically on FACIAL skin conditions and care.
            Your goal is to explain skin conditions on the face in plain language and provide actionable next steps.
            
            RULES:
            1. Always identify as a screening assistant, NOT a doctor.
            2. CRITICAL: For the VERY FIRST message (when user hasn't spoken yet), you MUST read the "Context from Scan" below. Start your response by explicitly echoing back the SCAN RESULTS on the face (e.g., "I see you've scanned your face and I noticed some mild acne..."). 
            3. After confirming the visual findings in that first message, immediately ask ONE highly targeted clinical question about POTENTIAL/HIDDEN symptoms on their face that cannot be seen in the photo (e.g., "Does the area on your cheek itch or feel painful?", "How long have these facial spots been present?", "Is your facial skin feeling tighter than usual?"). Do NOT ask generic branching questions.
            4. In subsequent messages, structure your facial skincare advice and ask further questions if needed to narrow down the issue. 
            5. Once you have enough info, say "Analysis Complete:" and give final advice tailored to facial skin.
...
app.post('/api/clinics', async (req, res) => {
    try {
        ...
        const prompt = `
            Based on the coordinates Latitude: ${lat}, Longitude: ${lng}, identify 3 real, highly-rated dermatology or skin clinics that are located near this location (prefer clinics in that specific city or neighborhood).
            Do not make up fake clinics. If the exact coordinates are remote, find the nearest major city's top dermatologists.
            For each clinic, provide the name, an estimated distance string (e.g. "2.5 km"), and a valid Google Maps search URL (e.g. "https://www.google.com/maps/search/?api=1&query=Clinic+Name").

            Return ONLY a valid JSON array of objects in this exact format:
            [
              {
                "name": "Acme Dermatology Clinic",
                "distance": "1.2 km",
                "mapsUrl": "https://www.google.com/maps/search/?api=1&query=Acme+Dermatology+Clinic"
              }
            ]
        `;
        ...
        // Handle cases where AI wraps it in an object like { clinics: [...] } instead of raw array
        let parsed = JSON.parse(rawJson);
        if (parsed.clinics) parsed = parsed.clinics;
