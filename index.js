...
app.post('/api/clinics', async (req, res) => {
    try {
        ...
        const prompt = `
            Given these GPS coordinates: Latitude ${lat}, Longitude ${lng}.
            
            STEP 1: First, determine EXACTLY which city, town, or neighborhood these coordinates are in. This is critical for accuracy.
            STEP 2: Then find 3 real dermatology clinics or skin specialist clinics that are CLOSEST to this location. They MUST be in the SAME city or town - never recommend clinics in a different country or city far away.
            STEP 3: Prioritize proximity (closest first), then ratings.
            
            For each clinic return: name, estimated distance (e.g. "0.8 km"), and a Google Maps search URL.
            The Google Maps URL format: "https://www.google.com/maps/search/?api=1&query=CLINIC+NAME+CITY"
            
            Return ONLY a valid JSON object with a "clinics" array:
            {"clinics": [{"name": "...", "distance": "0.8 km", "mapsUrl": "https://..."}]}
        `;
