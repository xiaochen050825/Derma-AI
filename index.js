...
const geminiKeys = [
    "AIzaSyBV0MQJHslRFVymiMb0wM4lZ7gnQcx12Xk",
    "AIzaSyCgzHCc98t3yd4DcVX6lYt7tTX-Ca95KII",
    "AIzaSyCPzhSGHDxtKsE0DvYykPkimRC71w0AYt8",
    "AIzaSyCO2tgSVJnDX1UkbD3iO0FFFOrkDuFu1XU",
...
const groq = new Groq({ apiKey: "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" });
...
app.post('/api/analyze', async (req, res) => {
    try {
        ...
        // Safely strip the data URI prefix (e.g., "data:image/jpeg;base64,")
        const base64Data = imageBase64.replace(/^data:image\/[a-zA-Z+]+;base64,/, "");

        // Extract mime type or default to jpeg
        ...
        const prompt = `
            ...
            In addition, map the exact facial area shown in the image to ONE of the following precise string values: 'head', 'neck', 'shoulderL', 'shoulderR', 'chest', 'abdomen', 'pelvis', 'upperArmL', 'upperArmR', 'forearmL', 'forearmR', 'handL', 'handR', 'thighL', 'thighR', 'shinL', 'shinR', 'footL', 'footR'.
            ...
            2. If evaluating the face, determine if it is reasonably well-centered and not completely obscured/cut off. Set "good_angle" to true or false. If not a face, evaluate the general clarity of the facial area.
            ...
            - CRITICAL: In the "primaryInsight" field, you MUST explicitly state the facial area scanned and the main finding (e.g., "Scanning the face reveals mild acne." or "Scanning the forehead shows normal healthy skin.").
            ...
            for (const modelName of fallbackModels) {
                try {
                    ...
                    if (err?.status === 429) {
                        ...
                        continue retryLoop; // Skip to next key if this key throws 403 or other unexpected error
                    }
                }
            }
        }
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        const systemInstruction = `
            ...
            2. CRITICAL: For the VERY FIRST message (when user hasn't spoken yet), you MUST read the "Context from Scan" below. Start your response by explicitly echoing back the facial area scanned and the main findings (e.g., "I see you've scanned your face and I noticed some mild acne."). 
            3. After confirming the visual findings in that first message, immediately ask ONE highly targeted clinical question about POTENTIAL/HIDDEN symptoms that cannot be seen in the photo (e.g., "Does it itch or feel painful?", "How long have these spots been present?", "Is this happening anywhere else on your face?"). Do NOT ask generic branching questions.
...
// 4. Nearby Clinics Finder (/api/clinics)
app.post('/api/clinics', async (req, res) => {
    try {
        ...
        const prompt = `
            The user is located at latitude ${lat}, longitude ${lng} in Malaysia.
            Suggest 3 real, reputable dermatology clinics or hospitals near this location.
            For each, provide:
            1. "name": The name of the clinic
            2. "distance": Approximate distance or location area
            3. "link": A direct Google Maps search URL (e.g. "https://www.google.com/maps/search/?api=1&query=Clinic+Name+Location")

            Respond ONLY with a JSON array of these 3 objects:
            [
              { "name": "...", "distance": "...", "link": "..." }
            ]
        `;
        ...
        for (let attempt = 0; attempt < geminiKeys.length; attempt++) {
            ...
            for (const modelName of fallbackModels) {
                try {
                    ...
                    lastErr = err;
                    if (err?.status === 429) continue retryLoop;
                    else if ([400, 404, 500, 503].includes(err?.status)) continue;
...
// The frontend is hosted separately on Netlify.
// We only serve the API on this Render deployment.

app.get('/', (req, res) => {
    res.send('DermaAI Backend API is running!');
});
