...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // Build Gemini-style conversation contents
        const contents = [];
        // Add system context as the first user turn
        contents.push({ role: "user", parts: [{ text: systemInstruction + "\n\nPlease acknowledge you understand these instructions and are ready." }] });
        contents.push({ role: "model", parts: [{ text: "Understood. I am DermaAI Copilot, ready to assist with dermatology screening. I have the scan context loaded." }] });
        ...
        // If last message is from user or no messages, add a prompt for the AI
        if (messages.length === 0) {
            contents.push({ role: "user", parts: [{ text: "Please analyze the scan results and begin the consultation." }] });
        }

        // Retry across keys and models
        let response = null;
        let lastErr = null;
        const startIndex = currentGeminiKeyIndex;
        ...
        for (let attempt = 0; attempt < geminiKeys.length; attempt++) {
            ...
            for (const modelName of fallbackModels) {
                try {
                    console.log(`[Chat] Trying model ${modelName} with key ending ...${geminiKeys[keyIndex].slice(-4)}`);
                    response = await ai.models.generateContent({
                        model: modelName,
                        contents: contents,
                    ...
                    lastErr = err;
                    if (err?.status === 429) {
                        console.warn(`[Chat] Rate limit on key ...${geminiKeys[keyIndex].slice(-4)}, next key...`);
                        continue retryLoop;
                    } else if ([400, 404, 500, 503].includes(err?.status)) {
                        console.warn(`[Chat] Model ${modelName} unavailable (${err.status}), next model...`);
                        continue;
                    } else {
                        console.warn(`[Chat] Unexpected error (${err?.status}):`, err.message);
                        continue retryLoop;
                    }
                }
            }
        ...
        res.json({ reply: reply || "I am currently offline." });
    } catch (err) {
...
app.post('/api/synthesize', async (req, res) => {
    try {
        ...
        for (let attempt = 0; attempt < geminiKeys.length; attempt++) {
            ...
            for (const modelName of fallbackModels) {
                try {
                    console.log(`[Synthesize] Trying model ${modelName} with key ending ...${geminiKeys[keyIndex].slice(-4)}`);
                    response = await ai.models.generateContent({
                        model: modelName,
                    ...
                    lastErr = err;
                    if (err?.status === 429) {
                        console.warn(`[Synthesize] Rate limit on key ...${geminiKeys[keyIndex].slice(-4)}, next key...`);
                        continue retryLoop;
                    } else if ([400, 404, 500, 503].includes(err?.status)) {
                        console.warn(`[Synthesize] Model ${modelName} unavailable (${err.status}), next model...`);
                        continue;
                    } else {
                        console.warn(`[Synthesize] Unexpected error (${err?.status}):`, err.message);
                        continue retryLoop;
                    }
                }
            }
